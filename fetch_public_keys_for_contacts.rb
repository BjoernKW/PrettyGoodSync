...
input.each_line do |line|
	if line =~ /:(\S+@\S+)\s+$/
		...
		begin
			uri = URI.parse("http://pgp.mit.edu:11371/pks/lookup?search=#{URI::encode(email_address)}&op=index")
...
key_queries.each do |key_id, key_query|
	begin
	  unless key_query.nil? || key_query == ''
		  ...
		  key_uri = URI.parse("http://pgp.mit.edu:11371/pks/lookup?op=get&search=#{key_query}")
